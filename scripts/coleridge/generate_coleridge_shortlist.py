...
data_dir = proj_root / "data/coleridgeinitiative/"
json_source_dir = data_dir / "coleridgeinitiative-show-us-the-data" / "train"

shortlist_dir = data_dir / "shortlist"
shortlist_dir.mkdir(parents=True, exist_ok=True)

COLERIDGE_RANKING_DATA_DIR = proj_root / "data/training/ranking/coleridge"


def build_shortlist(doc_path: Path, query: str, k: int = 25, skip_urls: bool = True, min_sentence_len: int = 25, vectorstore_cls = "Qdrant"):
    loader = structured_json.StructuredJSONLoader(doc_path, jq_schema=".[].text")
    docs = loader.load()
...
def main(num_docs: int = 100, k: int = 25, force: bool = False):
    ...
    for json_path in tqdm(json_list):
        if json_path.exists():
            ...
            ranking_file = COLERIDGE_RANKING_DATA_DIR / f"{json_path.stem}.json"
            ...
            if not shortlist_fname.exists() or force:
                try:
                    json_data = build_shortlist(json_path, query, k=k, skip_urls=True, min_sentence_len=25, vectorstore_cls="Qdrant")
            ...
            coleridge_docs.append(json_path)

        if len(coleridge_docs) >= num_docs:
            break


if __name__ == "__main__":
    # python -m scripts.coleridge.generate_coleridge_shortlist --num_docs=100 --k=50 --force
    fire.Fire(main)
